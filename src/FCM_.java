...
public class FCM_ implements PlugIn {
	...
	public void run(String arg) {
		...
		impseg = NewImage.createImage("Image segmentÈe par FCM", width, height, 1, 24, 0);
		...
		demande = JOptionPane.showInputDialog("Nombre itÈration max : ");
		int itermax = Integer.parseInt(demande);

		demande = JOptionPane.showInputDialog("Valeur du seuil de stabilitÈ : ");
		valeur_seuil = Double.parseDouble(demande);

		demande = JOptionPane.showInputDialog("Randomisation amÈliorÈe ? ");
		...
		// RÈcupÈration des donnÈes images
		l = 0;
		for (i = 0; i < width; i++) {
			for (j = 0; j < height; j++) {
		...
		// Initialisation des centroÔdes (alÈatoirement )
		...
		// Initialisation des degrÈs d'appartenance
		// A COMPLETER
		...
		while ((iter < itermax) && (stab > seuil)) {
			...
			// Affichage de l'image segmentÈe
